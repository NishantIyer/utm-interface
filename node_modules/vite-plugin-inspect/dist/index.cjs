...
const DIR_DIST = typeof __dirname !== "undefined" ? __dirname : node_path.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href))));
...
function PluginInspect(options = {}) {
  ...
  const idMap = {};
  const idMapSSR = {};
  function collectMiddlewarePerf(middlewares) {
  ...
  function hijackPlugin(plugin2) {
    hijackHook(plugin2, "transform", async (fn, context, args, order) => {
      ...
      if (filter(id) && result != null) {
        ...
        map[id].push({ name: plugin2.name, result, start, end, order });
      }
      return _result;
    });
    hijackHook(plugin2, "load", async (fn, context, args) => {
      ...
      const map = ssr ? transformMapSSR : transformMap;
      if (filter(id) && result != null)
        map[id] = [{ name: plugin2.name, result, start, end }];
      return _result;
    });
  ...
  function resolveId(id = "", ssr = false) {
    if (id.startsWith("./"))
      id = node_path.resolve(config.root, id).replace(/\\/g, "/");
  ...
  async function generateBuild() {
    ...
    await Promise.all([
      ...
      dumpModuleInfo(node_path.join(reportsDir, "transform"), transformMap),
      dumpModuleInfo(node_path.join(reportsDir, "transform-ssr"), transformMapSSR, true)
    ]);
    return targetDir;
  }
  ...
  const plugin = {
    ...
    async buildEnd() {
      ...
      console.log(c__default.green("Inspect report generated at"), c__default.dim(`${dir}`));
      if (_open && !isCI)
