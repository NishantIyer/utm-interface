...
// node_modules/.pnpm/@antfu+utils@0.7.4/node_modules/@antfu/utils/dist/index.mjs
...
function getPageFiles(path, options, pageOptions) {
  ...
  const files = import_fast_glob.default.sync(pattern, {
    ...
    onlyFiles: true,
    cwd: path
  }).map((p) => slash((0, import_path2.join)(path, p)));
  return files;
}
...
var PageContext = class {
  ...
  async addPage(path, pageDir) {
    ...
    for (const p of toArray(path)) {
      ...
      if (!extension)
        continue;
      const route = slash((0, import_path4.join)(pageDir.baseRoute, p.replace(`${pageDirPath}/`, "").replace(`.${extension}`, "")));
